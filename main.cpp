...
int main() {
    ...
    //Diddy Riese - Ami Sushi
    //1061 Broxton Avenue - Headlines!
    //John Wooden Center - Wooden West
    
    Navigator nav;
    assert(nav.loadMapData("/Users/akshaysmit/Desktop/CS32/Proj4/data.txt"));
    
    vector<NavSegment> directions;
    
    assert(nav.navigate("Cartier", "Resnick Neuropsychiatric Hospital", directions) == NAV_SUCCESS);
    
    for (int i = 0; i < directions.size(); i++) {
        
        if (directions[i].m_command == NavSegment::PROCEED) {
            cerr << "PROCEED" << endl;
            cerr << "Start: " << directions[i].m_geoSegment.start.latitudeText << " " << directions[i].m_geoSegment.start.longitudeText << endl;
            cerr << "End: " << directions[i].m_geoSegment.end.latitudeText << " " << directions[i].m_geoSegment.end.longitudeText << endl;
            cerr << "Direction: " << directions[i].m_direction << endl;
            cerr << "Distance: " << directions[i].m_distance << endl;
            cerr << "Street Name: " << directions[i].m_streetName << endl;
            cerr << endl;
        }
        else {
            cerr << "TURN" << endl;
    ...
    assert(nav.navigate("1061 Broxton Avenue", "LOL", directions) == NAV_BAD_DESTINATION);
    assert(nav.navigate("LOL", "1061 Broxton Avenue", directions) == NAV_BAD_SOURCE);
}
*/